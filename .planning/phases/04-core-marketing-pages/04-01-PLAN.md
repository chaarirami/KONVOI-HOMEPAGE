...
Do NOT install `@types/preact` — Preact 10 ships its own types.
  ...
  <acceptance_criteria>
    ...
    - `pnpm check:astro` exits 0 (no type errors introduced by the install)
  </acceptance_criteria>
  <done>pnpm install succeeds; astro.config.ts has Preact integration; pnpm check:astro passes</done>
</task>
